  ...
  \begin{itemize}
    ...
    \item In Calyx, \textbf{\textit{components}} correspond to hardware modules (with input and output ports). Each \textit{component} has three distinct sections:
          ...
          \begin{figure}[H]
            ...
            \end{minipage}
          \end{figure}
    \item Each \code{group} has \code{go} and \code{done} ports. \code{go} triggers execution of the \code{done} notifies when it finishes. The \code{control} section uses those signals to orchestrate \code{group} execution.
