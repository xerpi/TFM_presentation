...
\begin{frame}{\code{CodeGen_MLIR}: Halide IR to MLIR translation}
  \begin{itemize}
    \item Kernel arguments converted into \code{func}'s \codecpp{FuncOp}:
          \begin{enumerate}
            ...
            \item Buffer arguments: two MLIR arguments are generated:
                  \begin{enumerate}
                    \item 64-bit integer argument: base offset of the buffer within the assigned AXI interface. Written by the host code prior to kernel execution.
                    \item \codecpp{MemRefType} argument, \code{Memref} dialect, needed by MLIR to perform load/store accesses. Gets converted into a \textit{Calyx external memory interface} later on.
                  \end{enumerate}
          \end{enumerate}
    \item TODO
  \end{itemize}
